...
import { View, Text, TouchableOpacity, StyleSheet, FlatList, TextInput, RefreshControl, SafeAreaView, Image, ScrollView, Dimensions } from 'react-native';
...
const HomeScreenUser = () => {
  ...
  const loadTasks = async () => {
    try {
      ...
      if (response.data.success) {
        const userInitials = `${user.lastName} ${initials}`;
        const userTasks = response.data.tasks.filter(task => task.assignedTo === userInitials);
        const sortedTasks = sortTasks(userTasks, sortOption);
  ...
  const TaskItemLarge = ({ id, title, description, priority, createdBy, creatorRole, createdAt, dueDate, status }) => (
    <TouchableOpacity style={styles.taskItem} onPress={() => navigation.navigate('TaskScreen', { taskId: id })}>
      <View style={{ flex: 1 }}>
        ...
        <View style={{ flexDirection: 'row' }}>
          <View style={{
            paddingVertical: 5, paddingHorizontal: 10, marginVertical: 5, borderWidth: 1, borderRadius: 5, alignItems: 'center', justifyContent: 'center', alignSelf: 'flex-start',
          ...
          <View style={{
            paddingVertical: 5, paddingHorizontal: 10, marginVertical: 5, marginLeft: 5, borderWidth: 1, borderRadius: 5,
